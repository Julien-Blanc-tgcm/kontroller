...
void registerTypes()
{
	...
	assert(ret);
	ret = qmlRegisterUncreatableType<PlaylistService>(
	    qmlprefix, 1, 0, "PlaylistService", "PlaylistService is not creatable from qml");
	assert(ret);
	...
	ret = qmlRegisterType<ImageService>(qmlprefix, 1, 0, "ImageService");
	assert(ret);
	ret = qmlRegisterType<ImageControl>(qmlprefix, 1, 0, "ImageControl");
	assert(ret);
